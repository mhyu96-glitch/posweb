  ...
  const { data: settings, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .single();
      // .single() throws an error if no row is found, which is fine
      // but we can handle it gracefully if we want.
    ...
    <div className="container mx-auto p-4 md:p-6">
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            Informasi ini akan ditampilkan pada struk pembayaran Anda.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField control={form.control} name="shop_address" render={({ field }) => (
                <FormItem>
                  <FormLabel>Alamat Toko</FormLabel>
                  <FormControl><Textarea placeholder="Masukkan alamat toko" className="resize-none" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="shop_phone" render={({ field }) => (
                <FormItem>
